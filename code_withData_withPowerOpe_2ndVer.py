...
operators = [plus_btn, minus_btn, multiply_btn, divide_btn, parent_left_btn, parent_right_btn, exponent_btn, root_btn]
...
def evaluate_expression():
    ...
    for num in nums:
        ...
        if num.x > -1 and num.y > HEIGHT // 2:
            # get the number
            for special_operator_clone in special_operator_clones:
                if num in [special_operator_clone.first_operand, special_operator_clone.second_operand]:
                    used_num += 1
                    ignore = True
                    break
    ...
    for operator_clone in operator_clones:
        if operator_clone.image != "power_template" and operator_clone !="root_template":
            expression_ls[int(operator_clone.x // 20)] = parse_operator(operator_clone.image)

        for special_operator in special_operator_clones:
            if special_operator.image == "power_template":
                if special_operator.first_operand is None or special_operator.second_operand is None:
                    error_text = "exponent expression is wrong"
                else:

                    num_1 = special_operator.first_operand.image[4]
                    num_2 = special_operator.second_operand.image[4]
                    print("power operator:", num_1, num_2)
                    expression_ls[int(special_operator.x // 20)] = str(num_1) + "**" + str(num_2)
            elif special_operator.image == "root_template":
                if special_operator.first_operand is None or special_operator.second_operand is None:
                    error_text = "root expression is wrong"
                else:

                    num_1 = special_operator.first_operand.image[4]
                    num_2 = special_operator.second_operand.image[4]
                    print("root operator:", num_1, num_2)
                    expression_ls[int(special_operator.x // 20)] = str(num_2) + "**(1/" + str(num_1) + ")"
        ...
        for i in range(len(operators)):
            if operators[i].collidepoint(pos):
                ...
                if operators[i].image == "power":
                    ...
                    special_operator_clones.append(ope_clone)
                    # drag_power_operator_clone_index = len(special_operator_clones) - 1
                elif operators[i].image == "root":
                    ope_clone = Root_operator("root_template")
                    ope_clone.scale = operators[i].scale * 4
                    special_operator_clones.append(ope_clone)
                else:
...
def on_mouse_up(pos):
    ...
    if dragging:
        dragging = False
        # move the operators back to upper side of window and it will disappear
        if drag_operator_clone_index > -1 and pos[1] < HEIGHT // 2:
            disappear_operator_clone_index = drag_operator_clone_index
        elif drag_operator_clone_index > -1 and pos[1] >= HEIGHT // 2:
            # if the operator is exponent operator, update its first_operand and second_operand
            ope_clone = operator_clones[drag_operator_clone_index]
            if ope_clone.image == "power_template" or ope_clone.image == "root_template":
                if ope_clone.first_operand != None:
                    if not ope_clone.is_first_operand(ope_clone.first_operand):
                        ope_clone.first_operand = None

                if ope_clone.second_operand != None:
                    if not ope_clone.is_second_operand(ope_clone.second_operand):
                        ope_clone.second_operand = None

                print("after moving power operator:", ope_clone.first_operand, ope_clone.second_operand)

        # if the current dragging sprite is number, check if it becomes an operand of exponent operation
        if drag_num_index > -1:
            num = nums[drag_num_index]
            for operator in special_operator_clones:
                if operator.colliderect(num):
                    if operator.first_operand is not None:
                        error_text = "could not have two numbers in the same position."
                    else:
                        if operator.is_first_operand(num):
                            operator.first_operand = num

                    if operator.second_operand is not None:
                        error_text = "could not have two numbers in the same position."
                    else:
                        if operator.is_second_operand(num):
                            operator.second_operand = num
                    print(operator.first_operand)
                    print(operator.second_operand)
                    break
                else:
                    # if it is not colliding with the power_operator, need to update the
                    # power operator's first_operand and second_operand
                    if operator.first_operand is num:
                        print("detach the first operand")
                        operator.first_operand = None
                    if operator.second_operand is num:
                        print("detach the second operand")
                        operator.second_operand = None
